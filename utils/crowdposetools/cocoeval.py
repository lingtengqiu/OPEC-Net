...
class COCOeval:
    ...
    def summarize(self):
        ...
        def _summarizeKps():
            ...
            stats[2] = _summarize(1, maxDets=20, iouThr=.55)
            stats[3] = _summarize(1, maxDets=20, iouThr=.6)
            stats[4] = _summarize(1, maxDets=20, iouThr=.65)
            stats[5] = _summarize(1, maxDets=20, iouThr=.7)
            stats[6] = _summarize(1, maxDets=20, iouThr=.75)
            stats[7] = _summarize(1, maxDets=20, iouThr=.8)
            stats[8] = _summarize(1, maxDets=20, iouThr=.85)
            stats[9] = _summarize(1, maxDets=20, iouThr=.9)
            stats[10] = _summarize(1, maxDets=20, iouThr=.95)

            return stats[0],stats[1],stats[6]
        if not self.eval:
